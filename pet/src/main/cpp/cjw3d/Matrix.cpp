...
//static float sTemp[] = new float[32];
 ...
 */
void Matrix::multiplyMM(float result[], int resultOffset, float lhs[],
		int lhsOffset, float rhs[], int rhsOffset) {
	float* l = lhs + lhsOffset;
	float* r = rhs + rhsOffset;
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glLoadMatrixf(l);
	glMultMatrixf(r);
	glGetFloatv(GL_MODELVIEW_MATRIX, result + resultOffset);
	glPopMatrix();
...
void  Matrix::multiplyMV(float resultVec[],
	...
	for(int i=0;i<4;i++){
		resultVec[resultVecOffset+i]=0;
		for(int j=0;j<4;j++){
			resultVec[resultVecOffset+i]+=lhsMat[lhsMatOffset+i+j*4]*rhsVec[rhsVecOffset+i];
		}
 ...
 */
bool Matrix::invertM(float mInv[], int mInvOffset, float m[], int mOffset) {
 ...
 * @param zFar
 */
void Matrix::perspectiveM(float m[], int offset, float fovy, float aspect,
		float zNear, float zFar) {
	float f = 1.0f / (float) tan(fovy * (3.14f / 360.0));
 ...
 */
void Matrix::scaleM(float sm[], int smOffset, float m[], int mOffset, float x,
		float y, float z) {
	for (int i = 0; i < 4; i++) {
		int smi = smOffset + i;
		int mi = mOffset + i;
 ...
 */
void Matrix::translateM(float tm[], int tmOffset, float m[], int mOffset,
 ...
 * @param z scale factor z
 */
void Matrix::rotateM(float rm[], int rmOffset, float m[], int mOffset, float a,
		float x, float y, float z) {
	//setRotateM(sTemp, 0, a, x, y, z);
	//glmultiplyMM (rm, rmOffset, m, mOffset, sTemp, 0);
 ...
 * @param z scale factor z
 */
void Matrix::setRotateM(float rm[], int rmOffset, float a, float x, float y,
		float z) {
	rm[rmOffset + 3] = 0;
	rm[rmOffset + 7] = 0;
	rm[rmOffset + 11] = 0;
 ...
 */
void Matrix::setRotateEulerM(float rm[], int rmOffset, float x, float y,
		float z) {
	x *= (float) (PI / 180.0f);
	y *= (float) (PI / 180.0f);
	z *= (float) (PI / 180.0f);
 ...
 */
void Matrix::setLookAtM(float rm[], int rmOffset, float eyeX, float eyeY,
